...
func TestPermissionExplanationTracesMembershipAndResourceInheritance(t *testing.T) {
	...
	_ = tx.AddAssignmentExtended(ctx, NamespaceBuild, "p1", "g-admin", 1, 0, 0, 0, 1, 0)
	_ = tx.AddAssignmentExtended(ctx, NamespaceBuild, "p1/Shared", "g-team", 0, 1, 0, 0, 0, 1)
...
func TestPermissionExplanationHandlesMembershipCycles(t *testing.T) {
	...
	_ = tx.AddPermissionAction(ctx, NamespaceBuild, 1, "ViewBuilds", "View builds")
	_ = tx.AddAssignmentExtended(ctx, NamespaceBuild, "p1", "u-1", 0, 0, 0, 0, 1, 0)
	_ = tx.AddAssignmentExtended(ctx, NamespaceBuild, "p1", "g-2", 1, 0, 0, 0, 1, 0)
	_ = tx.Commit()
	_ = s.CompleteRun(ctx, runID, tx.Counts())
