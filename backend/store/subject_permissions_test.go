...
func TestSubjectPermissionsByRunReturnsAzureDevOpsEffectiveResults(t *testing.T) {
	...
	_ = tx.AddPermissionAction(ctx, NamespaceBuild, 2, "EditBuildDefinition", "Edit build pipeline")
	_ = tx.AddAssignmentExtended(ctx, NamespaceBuild, "p1/Shared/12", "u-1", 0, 0, 1, 0, 3, 0)
	_ = tx.Commit()
...
func TestSubjectPermissionsByRunIncludesExplicitDeny(t *testing.T) {
	...
	_ = tx.AddPermissionAction(ctx, NamespaceBuild, 1, "ViewBuilds", "View builds")
	_ = tx.AddAssignmentExtended(ctx, NamespaceBuild, "p1", "g-1", 0, 1, 0, 0, 0, 1)
	_ = tx.Commit()
	_ = s.CompleteRun(ctx, runID, tx.Counts())
